...
- Upstream edits to the deleted `release.yml` / `upd.yml` produce modify/delete conflicts with one correct resolution, so `auto-update.yml` re-deletes exactly those two paths. Any other conflicting path aborts the merge and fails the run for a human.
